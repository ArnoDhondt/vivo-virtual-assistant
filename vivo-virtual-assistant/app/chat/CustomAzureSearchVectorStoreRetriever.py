...
class CustomAzureSearchVectorStoreRetriever(BaseRetriever):
    ...
    def sort_with_date_relevancy(self, docs: List[Document], sorted_types: List[str]):
        ...
        sortable = [(i, doc) for i, doc in enumerate(docs) if 'type' in doc.metadata and doc.metadata['type'] in sorted_types]
        ...
        for i, result_doc in enumerate(result_docs):
            if  'type' in result_doc.metadata and result_doc.metadata['type']  in sorted_types:
